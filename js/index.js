	...
	$(function(){
		...
		$(".selected_animation").find("li").on("mouseover",function(){
			$(this).stop().animate({
				width:700
	...
	//ajax拼接JSON
	var oBox = document.querySelector(".content_modular_1_list")
	ajaxGet("http://localhost/dome_biyao/js/index.json",fn_succ);
	
	function fn_succ(json){
		var html = "";
		for(var i = 0 ; i < json.length;i++){
			html += `<li>
						<a href=""><img src="${json[i].img}" alt="" /></a>
						<dl>
							<dt>${json[i].name}</dt>
							<dd>${json[i].price}</dd>
						</dl>
...
//				console.log(zx+"："+zy)
				...
				$(".nav_right_list_list").eq(i).css({display:"block",top:zy - $("body").scrollTop() + 45,left:zx - 30})
